...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
       logging.info(f"NOTE HTTP trigger function processed a request with  note_id: {req_body['PrimaryEntityId']}.")
       ...
       return copy_note_attachment_to_blob_container(crm_organization_URI, note_id, oauth_token, error_messages)
        ...
        func.HttpResponse(json.dumps(error_messages, ensure_ascii=False), mimetype="application/json",  status_code= 401)
...
def copy_note_attachment_to_blob_container(crm_organization_URI, note_id, oauth_token, error_messages):
    ...
    error_in_work_order_request = error_in_note_request = False
    note_response = {}
    rest_api_URL = crm_organization_URI+'/api/data/v9.1' #full path to D365 Field Service REST api endpoint
       
    metadata_tags_values = {}
  
    if(oauth_token!=''):
        error_in_note_request = get_note(rest_api_URL, crm_request_headers, note_id, note_response, error_messages)
    
    if  not error_in_note_request:
        if work_order_note_contains_video_document(note_response):
            
            list_of_metadata_definition = get_metadata_definition(os.environ["STORAGE_ACCOUNT_NAME"], os.environ["STORAGE_CONTAINER"], os.environ["CONFIG_FILE"], error_messages)
            logging.info(f"LIST OF METADATA DEFINITIONS:{list_of_metadata_definition}")
            
            metadata_tags_values = get_metadata_values(crm_organization_URI, crm_request_headers,list_of_metadata_definition, note_response, error_messages) 
            
            note_video_content = download_note_attachment(rest_api_URL, note_id, file_request_headers, error_messages)
            ...
            logging.info(f"ENV VARIABLES:{os.environ['STORAGE_ACCOUNT_NAME']}, {os.environ['STORAGE_CONTAINER']}")
            error_saving_attachment = save_update_note_attachment_to_storage_container(note_response, metadata_tags_values, note_video_content,  os.environ["STORAGE_ACCOUNT_NAME"], os.environ["STORAGE_CONTAINER"], error_messages)
        

    if error_in_note_request or error_in_work_order_request or error_saving_attachment:  
        return func.HttpResponse(json.dumps(error_messages, ensure_ascii=False), mimetype="application/json",  status_code=401)
    else:    
...
def get_metadata_values(base_url, crm_request_headers, list_of_metadata_definition, note_response, error_messages):
    ...
    for metadata_definition in list_of_metadata_definition:  
        tags_values = get_tags_from_definition(note_response, metadata_definition, base_url, crm_request_headers, error_messages)
...
def get_oauth_token_response(resource):
    ...
    auth_token_result = requests.post(auth_token_endpoint, json=oauth_data_params)
    return auth_token_result


def get_note(rest_api_prefix, rest_headers, note_id, note_response, error_messages):
    try:
            note_web_api_query = rest_api_prefix+f"/annotations({note_id})?$select=isdocument,objecttypecode,_objectid_value,mimetype,createdon,filesize,filename&$expand=createdby($select=fullname)" #web api query (include leading /)
            note_res = requests.get(note_web_api_query, headers=rest_headers)
            note_parsed_res = note_res.json()

            note_response["note_id"] = note_id
            note_response["is_document"] = bool(note_parsed_res["isdocument"])
            note_response["object_type"] = note_parsed_res["objecttypecode"]
            note_response["work_order_id"] =  note_parsed_res["_objectid_value"]
            note_response["mime_type"] = note_parsed_res["mimetype"]
            note_response["created_on"] = note_parsed_res["createdon"]
            note_response["filesize"] = note_parsed_res["filesize"]
            note_response["filename"] = note_parsed_res["filename"]
            note_response["created_by"] = note_parsed_res["createdby"]["fullname"]           

            return False
    except Exception as ex:
        logging.error(f"NOTE After NOTE EXCEPTION:{ex}")
        error_messages["get_note"] = str(ex)
        return True

 

def get_metadata_definition(account_name, container, file_name, error_messages):
    file_content_response = get_json_file_from_blog(account_name, container, file_name, error_messages)
...
def get_json_file_from_blog( account_name, container, file_name, error_messages):
    ...
    if storage_auth_token_result.status_code != 200:
        ...
        logging.error(f"get_json_file_from_blog  - OAUTH STORAGE ERROR: {error_messages['get_json_file_from_blog-oauth_storage_request']}")
    else:    
        ...
        get_blob_result = requests.get(blob_storage_endpoint,headers=storage_request_headers)
        if get_blob_result.status_code != 200:
            ...
            logging.error(f"get_json_file_from_blog method - get Blob request : {error_messages['get_json_file_from_blog']}")
...
def get_tags_from_definition(note_response, tags_definitions, base_url, rest_headers,  error_messages):   
    ...
    try:
        ...
        object_rest_api_query= tags_definitions["query"].format(**format_params)
        
        record_url = tags_definitions["record_url"]

        query_response = requests.get(object_rest_api_query, headers=rest_headers)
        ...
        for tag_definition in tags_definitions["tags"]:
            ...
            if (tag_type is not None and tag_type == "URL"):
                ...
                tag_with_value = get_url_tag_value(query_response_json, tag_key ,record_url, base_url, tag_entity, tag_field)
                
            else:
                tag_with_value  = get_tag_value(query_response_json, tag_key, tag_field)
...
def get_url_tag_value(query_json_response, tag_key, record_url, base_url, tag_entity, tag_field_path):
    ...
    format_params={"base_url": base_url, "ent_name": tag_entity, "ent_id": tag_id_value}
...
def work_order_note_contains_video_document(note_response):
    return note_response["object_type"] == "msdyn_workorder" and note_response["is_document"] and "video" in note_response["mime_type"]
...
def download_note_attachment(rest_api_prefix, note_id, rest_headers, error_messages):
   ...
   try:
        file_rest_api = rest_api_prefix + f"/annotations({note_id})/documentbody/$value"
...
def save_update_note_attachment_to_storage_container(note, metadata_tags, note_attachment, account_name, container, error_messages):
    ...
    if storage_auth_token_result.status_code != 200:
        ...
        logging.error(f"OAUTH STORAGE ERROR: {error_messages['oauth_storage_request']}")
        return  False
    ...
    put_blob_result = requests.put(blob_storage_endpoint, data=note_attachment,headers=storage_request_headers)

    logging.info(f"PUT STORAGE HTTP CODE: {put_blob_result.status_code}, RESPONSE:{put_blob_result.text}")
    if (put_blob_result.status_code!=201):
        error_messages['save_update_note_attachment_to_storage_container']=f"HTTP CODE: {put_blob_result.status_code} - ERROR:{put_blob_result.text}"
        logging.error(f"PUT STORAGE BLOB ERROR: {error_messages['save_update_note_attachment_to_storage_container']}")
        return  False
